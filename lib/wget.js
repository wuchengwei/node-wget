...
function download(src, output, options, __parentevent) {
    var downloader = __parentevent || new EventEmitter(),
        srcUrl,
        tunnelAgent,
        req;
    ...
    req = request({
        ...
        if (res.statusCode === 200) {
            ...
            req.abort(); // release resources to avoid EMFILE
            if (res.header.location) {
                try {
                    downloader(res.header.location, output, options, downloader)
                    return;
                } catch (e) {
                    // invalid url, we don't care
                }
            }
